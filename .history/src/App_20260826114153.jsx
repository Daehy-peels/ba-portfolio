...
export default function App() {
  ...
  return (
    <div
      className={`min-h-screen transition-all duration-500 ${darkMode ? "bg-slate-950 text-slate-100" : "bg-gradient-to-br from-emerald-50/50 via-white to-teal-50/30 text-slate-800"} font-sans selection:bg-emerald-500 selection:text-white`}
    >
      {/* Navigation Bar with Glassmorphism */}
      <nav
        className={`fixed top-0 left-0 right-0 z-50 border-b backdrop-blur-xl transition-all duration-300 ${darkMode ? "bg-slate-900/70 border-slate-800/80" : "bg-white/70 border-emerald-100/60"}`}
      >
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          <a
            href="#about"
            className="font-extrabold text-xl tracking-tight flex items-center gap-2 group"
          >
        ...
        {mobileMenuOpen && (
          <div
            className={`md:hidden border-b px-6 py-6 flex flex-col space-y-4 backdrop-blur-2xl transition-all ${darkMode ? "bg-slate-900/95 border-slate-800" : "bg-white/95 border-slate-200"}`}
          >
            <a
              href="#about"
      ...
      <section
        id="about"
        className="max-w-6xl mx-auto px-6 pt-36 pb-24 md:pt-48 md:pb-32 flex flex-col items-start relative"
      >
        ...
        <div
          data-aos="fade-up"
          className="inline-flex items-center gap-2 bg-emerald-100/80 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 text-xs font-semibold px-4 py-2 rounded-full uppercase tracking-wider mb-6 border border-emerald-200/80 dark:border-emerald-800/50 shadow-sm backdrop-blur-md"
        >
        ...
        <h1
          data-aos="fade-up"
          data-aos-delay="100"
          className="text-4xl md:text-7xl font-extrabold tracking-tight mb-6 max-w-4xl leading-[1.1]"
        >
        ...
        >
          I transform operational roadblocks into streamlined workflows,
          crystal-clear insights, and measurable ROI through rigorous
          requirements engineering and predictive analysis.
        </p>
      ...
      <section
        id="skills"
        className={`py-28 border-y transition-colors duration-500 relative ${darkMode ? "bg-slate-900/40 border-slate-800/80" : "bg-white/60 border-emerald-100/60 backdrop-blur-md"}`}
      >
        <div className="max-w-6xl mx-auto px-6">
          ...
          >
            ...
            <p className={darkMode ? "text-slate-400" : "text-slate-600"}>
              The professional frameworks and modern technologies I use to
              architect solutions.
            </p>
          </div>
      ...
      <section id="projects" className="max-w-6xl mx-auto px-6 py-28">
        ...
        >
          <div>
            ...
            <p className={darkMode ? "text-slate-400" : "text-slate-600"}>
              Real-world business transformation challenges solved through data.
            </p>
          </div>
          ...
          {filteredProjects.map((project, index) => (
            ...
            >
              ...
              <div>
                ...
                <div
                  className={`border rounded-2xl p-4 mb-6 text-xs font-medium transition-colors ${darkMode ? "bg-emerald-950/30 border-emerald-900/50 text-emerald-300" : "bg-emerald-50/60 border-emerald-100 text-emerald-900"}`}
                >
              ...
              <div>
                <div className="flex flex-wrap gap-1.5 mb-6">
                  {project.tools.map((tool, tIdx) => (
                    <span
                      key={tIdx}
                      className={`text-xs px-3 py-1 rounded-xl font-medium ${darkMode ? "bg-slate-800 text-slate-300" : "bg-slate-100 text-slate-600"}`}
                    >
                      {tool}
                    </span>
                  ))}
                ...
                <button
                  className={`w-full py-3.5 rounded-2xl text-sm font-medium transition-all duration-300 flex items-center justify-center gap-2 group-hover:shadow-lg ${darkMode ? "bg-slate-800 text-white hover:bg-emerald-600" : "bg-slate-900 text-white hover:bg-emerald-700"}`}
                >
                  View Full Report{" "}
                  <ExternalLink
                    size={14}
      ...
      <section
        id="contact"
        className={`py-28 transition-colors duration-500 relative overflow-hidden ${darkMode ? "bg-slate-900 border-t border-slate-800" : "bg-gradient-to-br from-emerald-900 via-emerald-950 to-teal-950 text-white"}`}
      >
        {/* Glow overlay */}
        ...
        >
          ...
          <p
            className={`max-w-xl mx-auto mb-10 text-base md:text-lg leading-relaxed ${darkMode ? "text-slate-400" : "text-emerald-100/90"}`}
          >
            Whether you're looking for an analyst to streamline complex
            workflows, optimize data pipelines, or drive strategic product
            rollouts, let's connect.
          </p>
      ...
      {/* Footer */}
      <footer
        className={`py-8 text-center text-xs transition-colors border-t ${darkMode ? "bg-slate-950 border-slate-900 text-slate-500" : "bg-slate-950 border-slate-900 text-emerald-400/60"}`}
      >
        <p>
          © {new Date().getFullYear()} • Built with React, Tailwind CSS, AOS &
          Smooth Animations.
        </p>
      </footer>
    </div>
